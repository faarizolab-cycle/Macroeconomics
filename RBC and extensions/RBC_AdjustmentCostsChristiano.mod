% RBC Model with Investment Adjustment Costs (Christiano et al. 2005)
...
% 2. Investment FOC (Explicit derivation required for Dynare)
% Marginal cost of investing = Marginal benefit of installed capital + Future adjustment savings
1 = q * ( 1 - (phi/2)*(I/I(-1) - 1)^2 - phi*(I/I(-1) - 1)*(I/I(-1)) ) 
    + beta * (C/C(+1)) * q(+1) * phi * (I(+1)/I - 1) * (I(+1)/I)^2;

% 3. q-Euler equation (Capital pricing dynamics)
q = beta * (C/C(+1)) * ( R(+1) + (1 - delta)*q(+1) );

% 4. Labor supply
W/C = theta * N^chi;

% 5. Capital accumulation
K = (1 - delta)*K(-1) + ( 1 - (phi/2)*(I/I(-1) - 1)^2 ) * I;
